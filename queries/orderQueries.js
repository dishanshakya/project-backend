...
const order = 'select orders.*, username, profile_pic_src, category_name from orders inner join users\
    on users.user_id = orders.user_id inner join category on orders.category = category.category_id where order_id = $1';
const viewComments = "select comments.*, users.username, users.profile_pic_src from comments join users on comments.user_id = users.user_id where order_id = $1"
const viewReplies = "select replies.*, users.username, users.profile_pic_src from replies join users on replies.user_id = users.user_id where comment_id = $1"
